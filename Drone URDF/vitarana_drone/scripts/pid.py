...
def PID_alt(roll, pitch, yaw,x,y, target, altitude, k_alt, k_roll, k_pitch, k_yaw, k_x, k_y, velocity, k_vel, flag): 
    ...
    if(speed.prop1 < 200): speed.prop1 = 200
    if(speed.prop2 < 200): speed.prop2 = 200
    if(speed.prop3 < 200): speed.prop3 = 200
    if(speed.prop4 < 200): speed.prop4 = 200
    # rospy.loginfo(speed) 
...
def position_controller(target_x, target_y, x, y, velocity, k_vel, flag):
    ...
    err_x = round(x,2) - round(target_x,2)
    err_y = round(y,2) - round(target_y,2)
    err_vel_x = vel_x - 0
    err_vel_y = vel_y - 0

    # if err_x<1 and err_x>-1: err_x = 0.0
    # if err_y<1 and err_y>-1: err_y = 0.0

    # print("Vel X Error = ", err_vel_x)

    x_err_pub = rospy.Publisher("/x_err", Float64, queue_size=100)
    x_err_pub.publish(err_x)
    y_err_pub = rospy.Publisher("/y_err", Float64, queue_size=100)
    ...
    if(dTime >=sample_time):
        ...
        if(iMem_vel_y<-10): iMem_vel_y=-10
        if(iMem_x>5): iMem_x = 5
        if(iMem_x<-5): iMem_x=-5
        # if(iMem_y>45): iMem_y = 45
        # if(iMem_y<-45): iMem_y=-45
    ...
    print("I Term Y = ",ki_y*iMem_y)
    print("D Term Y = ",kd_y*dMem_y)
